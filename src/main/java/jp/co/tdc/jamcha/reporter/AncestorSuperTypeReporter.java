...
import jp.co.tdc.jamcha.model.SuperType;
...
public class AncestorSuperTypeReporter {
    ...
    Row header() {
        return new Row(List.of(
            ...
            new Cell("ancestorSuperTypeQualifiedName")
        ));
    }
    ...
    Stream<Row> rows(SourceAnalyzeResult r, Type t) {
        return t.metadata().ancestorSuperTypes().stream().map(s -> row(r, t, s));
    }

    Row row(SourceAnalyzeResult r, Type t, SuperType s) {
        return new Row(List.of(
            new Cell(r.name()),
            ...
            new Cell(s.qualifiedName().value())
        ));
    }
}
